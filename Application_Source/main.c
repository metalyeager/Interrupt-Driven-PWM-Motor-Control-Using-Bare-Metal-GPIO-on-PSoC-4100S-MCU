...
// {
//     /* HF CLOCK divider init */
//     Cy_SysClk_ClkHfSetDivider(0u); // 0 - No Divider

//     init_peri_Clock_Config();

//     /* Configure P1.2 as GPIO default high and strong drive */
//     *((uint32_t *)0x40040100) |= (1 << 2);    // DR default 1 for P1.2
//     uint32_t reg = *((uint32_t *)0x40040108);
//     reg &= ~(0x7u << 6);   
//     reg |=  (6u << 6);
//     *((uint32_t *)0x40040108) = reg;
//     *((uint32_t *)0x40020100) = (8 << 8); // Assign GPIO to PWM output (HSIOM_PORT_SEL1), refer hsiom table 0x8 corresponds to tcpwmoutput
//     *((uint32_t *)0x40200000) &= ~(1 << 3);   // Disable Timer 3 (TCPWM_CTRL)
//     *((uint32_t *)0x402001C8) = 0;            // Clear counter (TCPWM3_COUNTER)
//     *((uint32_t *)0x402001E8) = 0x31;         // CC_MATCH_MODE / OVERFLOW_MODE 
//     *((uint32_t *)0x402001D4) = (timer_period - 1); // Period register (TCPWM3_PERIOD)
//     *((uint32_t *)0x402001CC) = 0;            // Compare register (TCPWM3_CC)
//     *((uint32_t *)0x402001C0) |= (4 << 24) | (1 << 3);
//     *((uint32_t *)0x40200000) |= (1 << 3);    // Enable Timer 3 (TCPWM_CTRL)
//     *((uint32_t *)0x40200008) |= (1 << 11);   // Reload trigger (TCPWM_CMD)
...
//         for (uint32_t cc = 0; cc <= timer_period; cc += steps)
//         {
//             *((uint32_t *)0x402001CC) = (cc >= timer_period) ? (timer_period - 1) : cc;
//             soft_delay(1800);
//         }
//         for (int32_t cc = (int32_t)timer_period; cc >= 0; cc -= steps)
//         {
//             *((uint32_t *)0x402001CC) = (cc <= 0) ? 0 : (uint32_t)cc;
//             soft_delay(1800);
//         }
